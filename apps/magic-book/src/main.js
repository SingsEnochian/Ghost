...
function turnLeaf(delta) {
    const current = Math.max(0, volumes.findIndex(volume => volume.id === activeId));
    ...
    dispatchMagicBookEvent(delta > 0 ? 'page-next' : 'page-previous', activeId, {volumeId: activeId});
...
function render() {
    ...
    root.querySelectorAll('[data-page-step]').forEach(button => button.addEventListener('click', () => { turnLeaf(Number(button.dataset.pageStep)); render(); }));
